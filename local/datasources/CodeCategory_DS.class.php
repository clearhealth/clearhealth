...
class CodeCategory_DS extends Datasource_Sql {
	...
	function CodeCategory_DS() {
		$labels = array('actions'=>false, 'category_name' => 'Name', 'num' => '# Codes');

		$this->setup(Celini::dbInstance(),
			array(
				'cols' => "category_name, count(*) num, c.code_category_id",
				'from' => 'code_category c inner join code_to_category ctc on c.code_category_id = ctc.code_category_id',
				'groupby' => 'c.code_category_id'
			),
			$labels);

		$this->addDefaultOrderRule('category_name','ASC',1);
