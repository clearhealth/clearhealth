...
CREATE TABLE `person` (
  ...
  PRIMARY KEY  (`person_id`),
  CONSTRAINT `person_ibfk_7` FOREIGN KEY (`person_id`) REFERENCES `ownership` (`id`)
...
CREATE TABLE `person_address` (
  ...
  CONSTRAINT `person_address_ibfk_3` FOREIGN KEY (`person_id`) REFERENCES `person` (`person_id`),
  CONSTRAINT `person_address_ibfk_4` FOREIGN KEY (`address_id`) REFERENCES `address` (`address_id`)
...
CREATE TABLE `reports` (
  ...
  PRIMARY KEY  (`id`)
) TYPE=InnoDB COMMENT='Report definitions TODO: change to Generic Seq';
