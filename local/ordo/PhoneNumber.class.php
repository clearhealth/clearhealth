...
class PhoneNumber extends ORDataObject{
	...
	function factory_phone_numbers($foreign_id = "") {
		...
		$phone_numbers = array();
		$p = new PhoneNumber();
		$sql = "SELECT * FROM  " . $p->_prefix . $p->_table . " WHERE foreign_id " .$foreign_id . " ORDER BY type";
		//echo $sql . "<bR />";
		$results = $this->_Execute($sql) or die ("Database Error: " . $this->_db->ErrorMsg());
		//echo "sql: $sql";
		$i = 0;
		while ($results && !$results->EOF) {
			$phone_numbers[$i] = new PhoneNumber();
			$phone_numbers[$i]->populate_array($results->fields);
			$i++;
			$results->MoveNext();
		}
