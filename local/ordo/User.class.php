...
class User extends Base_User {
   ...
   function users_factory($group="") {
                ...
                if (!empty($group)) {
                  ...
                  if ($group =="provider") {
                     //EW edited by eric 2-27-06
                     //I hard coded the number two here but this is bad practice and 
                     //should really be pulled dynamically
                     //$sql = "SELECT u.user_id FROM provider INNER JOIN user u USING ( person_id ) ";
                     
                     $sql = "SELECT distinct(u.user_id )FROM  provider INNER JOIN user u 
                              USING ( person_id ) 
                              INNER JOIN person_type pt
                              USING (person_id)
                              WHERE pt.person_type = \"2\"";
