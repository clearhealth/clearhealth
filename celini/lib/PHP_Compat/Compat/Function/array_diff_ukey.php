...
// +----------------------------------------------------------------------+
//
// $Id: array_diff_ukey.php,v 1.3 2004/11/21 17:36:53 aidan Exp $
...
if (!function_exists('array_diff_ukey')) {
    function array_diff_ukey()
    {
        $args = func_get_args();
        if (count($args) < 3) {
            trigger_error('Wrong parameter count for array_diff_ukey()', E_USER_WARNING);
            return;
        }
        ...
        for ($i = 0; $i !== $array_count; $i++) {
            if (!is_array($args[$i])) {
                trigger_error('array_diff_ukey() Argument #' . ($i + 1) . ' is not an array', E_USER_WARNING);
                return;
            }
        }
